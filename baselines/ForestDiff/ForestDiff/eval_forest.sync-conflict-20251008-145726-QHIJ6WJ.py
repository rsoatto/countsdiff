...
def evaluate_imputer(imputer, test_dataset, covariate_df_builder, impute_mask=None, num_samples=None, dropout_ratio=0.5):
    ...
    masked = ground_truth * (1-impute_mask).float()
    with torch.no_grad():
    ...
    if isinstance(imputed_data, np.ndarray):
        imputed_data = torch.from_numpy(imputed_data).to(ground_truth.device)
    ...
    #Compute scFID
    scfid.update(ground_truth.cpu().numpy(), covariate_df_builder(labels), True)
    scfid.update(imputed_data.cpu().numpy(), covariate_df_builder(labels), False)
...
def main():
    ...
    if args.missing_mask:
        impute_mask = torch.Tensor(np.load(args.missing_mask).astype(bool))
    else:
        impute_mask = None
    results = evaluate_imputer(forrestdiff_imputer, test_dataset, test_dataset.get_conditions_df(), impute_mask)
    return results
