"""
Parallel evaluator for scRNA-seq imputation sweeps across multiple GPUs.

This script mirrors scripts/eval_inference_pool.py but evaluates imputation on
scRNA-seq data using CountsdiffImputer.impute_data. It retains the same
parallelization and pooling structure (task queue per GPU), but the worker
generates imputed counts, then computes RMSE, R2, and scFID as in
scripts/all_imputation.ipynb.

Notes:
- Supports per-setting sharding. Each shard computes partial statistics and
  latent features, which are aggregated into final per-setting metrics.
"""
...
def worker(device_idx: int, device_str: str, task_queue: mp.Queue, progress_queue: mp.Queue, args, exp_output_path: str, results_dir: str, samples_output_path: str, shards_output_path: str, log_file_path: str):
    ...
    imputer = ReMDM.from_checkpoint(args.checkpoint_path, device=device_str)

    device = torch.device(device_str if torch.cuda.is_available() else 'cpu')

    val_data = SingleCellDataset(args.data_file, split = "val", condition_keys = args.cond_keys) 
    
    # Prepare scFID metric if possible (build from dataset + provided scvi model)
    scfid_metric = None
    try:
        gene_names = getattr(val_data, 'gene_names', None)
        if gene_names is not None and args.scvi_model_path is not None and os.path.exists(args.scvi_model_path):
            ...
            categorical_covariates = val_data.get_obs_dict(unique=True)
    ...
    while True:
        ...
        try:
            ...
            task_queue.task_done()
def preflight_or_die(args):
    import traceback
    try:
        # Sanity: paths
        assert os.path.exists(args.checkpoint_path), f"checkpoint_path not found: {args.checkpoint_path}"
        assert os.path.exists(args.data_file), f"data_file not found: {args.data_file}"
        if args.scvi_model_path:
            logging.info(f"scvi_model_path: {args.scvi_model_path} (exists={os.path.exists(args.scvi_model_path)})")

        # Model + dataset + 1 tiny impute on CPU
        imputer = ReMDM.from_checkpoint(args.checkpoint_path, device='cuda:0' if torch.cuda.is_available() else 'cpu')
        val_ds = SingleCellDataset(args.data_file, split="val", condition_keys=args.cond_keys)
        dl = DataLoader(val_ds, batch_size=2, shuffle=True)
        counts, labels, missing_masks = next(iter(dl))
        valid_mask = (~missing_masks).bool()
        impute_mask = (torch.rand_like(counts) < 0.1) & valid_mask
        masked = counts * (~impute_mask).float()
        # normalize labels into list[Tensor]
        if isinstance(labels, dict):
            labels = [labels[k] for k in sorted(labels)]
        elif not isinstance(labels, (list, tuple)):
            labels = [labels]
        _ = imputer.impute_data(counts=masked, valid_mask=valid_mask, impute_mask=impute_mask, labels=labels)
        print("Preflight OK.")
    except Exception:
...
def main():
    ...
    parser.add_argument('--checkpoint_path', required=True, help='remdm run checkpoint path (.pth file)')
    parser.add_argument('--data_type', type=str, required=True, choices=['heart', 'fetus'])
    ...
    parser.add_argument('--dropout-ratio', type=float, default=0.5, help='Random dropout ratio for imputation mask (applied on valid positions only)')
    ...
    progress_queue: mp.Queue = mp.SimpleQueue()
    ...
    while completed < total and any(p.is_alive() for p in procs):
    ...
    if args.setting_workers > 1:
        ...
        imputer_for_metrics = ReMDM.from_checkpoint(args.checkpoint_path, device=devices[0] if torch.cuda.is_available() else 'cpu')
        val_ds_for_metrics = SingleCellDataset(args.data_file, split = "val", condition_keys = args.cond_keys)
